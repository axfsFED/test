...
samples = ['szzs', 'zxbz', 'cybz', 'sz50', 'hs300', 'zz500']
...
def micro_statistics(start_date, end_date, target_index):
    ...
    df_result.to_sql(samples[indexs.index(target_index)], engine, if_exists='replace', index=True)  #增量入库
...
if __name__ == '__main__':
    ...
    for i in range(0, len(indexs)):
        t = threading.Thread(target=micro_statistics,args=(start_date,end_date,indexs[i]))
        threads.append(t)
